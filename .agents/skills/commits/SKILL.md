...
Add `#<Task-ID>` to the subject line to link commits to work items.
...
```
feat(orders): #ULF-420 add discount calculation to order total
...
```
fix(api): #ULF-421 handle missing customer_id in order creation
...
```
test(domain): #ULF-422 add tests for order confirmation edge cases
```
...
```
feat(api)!: #ULF-423 change order status enum values
